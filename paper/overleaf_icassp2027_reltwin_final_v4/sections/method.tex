...
RBEE adds $\lambda\JS(p_{q_+},\Pi p_{q_-})$, where $\Pi$ swaps candidate indices: exchange consistency, not architectural equivariance. Perfect candidate labels already satisfy this exchange.

SetPO adds 64 updates with six answers: both windows, their expanded versions, their union, and one spanning interval. Soft targets average set-IoU and symmetric best-match interval F1 (not one-to-one event F1). We compare it with RBEE continued for 64 updates to isolate the objective from additional training.
