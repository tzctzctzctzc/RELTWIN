...
**最终额外小诊断已冻结，尚未启动：** `experiments/protocols/RELTWIN_TIMING_STRESS_20260913.md`，提交 `efc5ca2`。这不是取消的 CompA 外部测试。针对构造中布局与静音时长耦合，只把静音模式统一，保留全部 80 段原音频中的声音、原查询和原布局。只取 `followed_by` 模板共 160 条，运行固定 current SFT seed 0 与 Cand seed 0，共 320 次推理、无训练；一个小时硬上限。等桥接队列 COMPLETE 后才启动，不能 GPU 并行抢卡。主指标是变化的 40 段 BA-first 音频上 Cand−SFT JointPairAcc；必须报告不利结果，不调参。该诊断完成后不再扩实验，收束稿件。新增脚本 `scripts/reltwin_timing_stress.py`，准备阶段逐样本检查原始重复波形和未变化布局的恒等性。
