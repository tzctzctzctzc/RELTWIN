...
| **Boundary Utility, frozen full run** | deterministic | **59.386** | **79.25** | **62.50** | latest point estimate; `+0.057` over E004 NOVA, CI crosses zero |
...
The Boundary Utility extension keeps E004 NOVA as the incumbent and applies an incumbent-relative local boundary action only when its calibrated gain lower bound clears the frozen margin. On the complete 400-row SpotSound-Bench evaluation it reaches **59.385511 mIoU**, `+0.056963` point over E004, with 20 wins, 15 losses, 365 ties, 36 switches, and zero new catastrophic regressions. The audio-group bootstrap 95% interval is `[-0.114667, +0.236260]`, so this remains a point-estimate improvement rather than a statistically significant claim. On all 6,649 Clotho-Moment rows it reaches **86.855592 mIoU** versus the **86.854263** official incumbent, with 5 wins, 4 losses, 6,640 ties, 9 switches, and zero new catastrophic regressions. Frozen reports are committed under [`results/nova_boundary_utility_v1/`](results/nova_boundary_utility_v1/).
