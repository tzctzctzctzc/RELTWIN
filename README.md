...
[当前论文 PDF](paper.pdf) · [修改报告](CHANGELOG.md) · [合作者交接](HANDOFF_NOTES.md) · [问题清单](ISSUES.md) · [LaTeX 主文件](main.tex)

**当前为 v36（2026-09-23）**：按作者要求基于 GitHub `b28b3e9` 做有限范围增量修改；摘要限定机制实验身份、统一 interval count、拆分相关工作段落、补充 Stage 2 与 KL 动机、校正图内一致性标签，并给作者姓名加入 ORCID 链接。实验、结论、表格和公式未改。按作者最新要求，新版统一保存为 `paper.pdf`，四页正文加第五页声明与文献，旧版由 Git 历史保留。以下 v35 说明作为历史记录；本轮详情以 CHANGELOG 与 HANDOFF_NOTES 为准。
