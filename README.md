...
`paper` 分支同时保存可编辑源码和当前编译稿。当前为 v14（P0-01 摘要与贡献段精炼稿），标题为 **RelTwin: Contrasting Locally Valid Timestamp Answers for Query-Specific Audio Grounding**。本轮仅精炼摘要和引言贡献段：突出问题、方法与主要发现，减少重复报数，实验设置与完整比较口径仍保留在实验部分。此前的 C01 配置补充和 Figure 1 改进全部保留。没有训练、重新推理或改动实验结果。待处理问题、作者信息占位及提交前检查统一记录在 `ISSUES.md`。
