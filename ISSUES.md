...
**当前是 v11 Figure 1 更新稿，不是已完成投稿检查的终稿。** 本轮基于 `paper` 的 `d33969b` 重绘主图，同步更新图注、对应案例说明和 `paper.pdf`；实验数据未变。旧版本由 Git 历史保存。编辑与编译方法见 [README.md](README.md)。
...
- 位置：PDF 第 4–5 页。
- 现状：技术内容在第 4 页结束，参考文献 [1]–[12] 在第 4 页开始，第 5 页续接 [12] 并列出 [13]–[17]。尚未形成作者要求的“前四页正文、第五页集中放文献”。
...
- 现状：Table 1 仍浮动到方法部分附近，早于结果段的首次引用；本轮重编译后 4.3 小节从第 4 页开始，原有该小节标题跨页问题已消失，整体表格位置仍待整理。
...
- 验证：`overview_data.tex` 与基础版本完全一致；目标与预测条形仍按线性时间轴绘制。图注解释实线/虚线候选、空心目标/实心预测与 set-IoU。独立图和五页论文均编译通过，三轮图像检查及实际双栏尺寸检查完成；推理条明确标为 unchanged。
- 原审阅：#55–#57。
...
- 当前 PDF SHA-256：`c7c0344bb3576d75202eae63828d8b9b60eb0562b2f0cd31592d13c38d48b622`。
