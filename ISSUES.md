...
**当前是 v11 主图精简与配色更新稿，不是已完成投稿检查的终稿。** 按作者要求撤去 Figure 2 结果流向图，并删除 Figure 1 的底部 inference pipeline 与 160 对整体统计；其余三栏内容保留。新配色取自 BLIP-2（ICML 2023）和 InstructBLIP（NeurIPS 2023）的矢量方法图，来源和色值见源码注释。修改保存在 `codex/reltwin-figure1-redesign`，尚未合入 `paper`。旧版本由 Git 历史保存。编辑与编译方法见 [README.md](README.md)。
...
- 验证：`overview_data.tex` 与基础版本完全一致；目标与预测条形仍按线性时间轴绘制。浅色对角单元格标正确候选、上/下箭头标训练偏好；虚线仅标训练区域，空心目标/实心预测与 set-IoU 的语义不变。当前精简版已按作者要求删除底部 inference pipeline 和右下整体统计，图注同步删去对应说明；独立图、灰度预览与五页论文均完成检查。
- 原审阅：#55–#57。
...
- 当前 PDF SHA-256：`e7d3e23694f6de810f55e0f3a14cf99bab85d94bc9678d6b7a42d414d47b61bc`。
