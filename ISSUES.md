...
当前论文：[tang.pdf](tang.pdf) · 目标会议：ICASSP 2027 · 更新日期：2026-09-14

**当前是 v11 审阅稿，不是已完成投稿检查的终稿。** 本次仅建立固定文件入口、整理问题并发布；PDF 内容没有修改，没有补实验。后续在本分支固定维护 `tang.pdf` 和 `ISSUES.md`，旧版本由 Git 历史保存。
...
- 位置：PDF 第 4–5 页。
- 现状：技术内容在第 4 页结束，但参考文献 [1]–[12] 已出现在第 4 页，第 5 页仅续列 [13]–[17]。尚未形成作者要求的“前四页正文、第五页集中放文献”。
...
- 现状：Table 1 浮动到方法部分附近，早于结果段的首次引用；4.3 小节标题和开头少量文字位于第 3 页末，随后跨至第 4 页。
...
- 文件名已处理：本分支统一用第一作者姓氏命名为 `tang.pdf`，不再使用带版本号的交付名称。[官方文件要求](https://cmsworkshops.com/ICASSP2027/papers/paper_kit.php)

### 当前已经通过、不要误列为缺陷的项目

v11 为 5 页 US Letter，约 286 KB；第 5 页没有技术内容。正文为 10 pt，表格与图注按 9 pt 排版；21 个字体均为嵌入、子集化的 Type 1 字体，无加密。摘要 127 词、4 个关键词、17 篇已引用文献。未发现页码、未解析引用、未解析交叉引用或 overfull box。模板文件与当时核验的官方文件一致；已有交付审计记录 69 项测试通过及独立编译核验。本次没有重新运行那 69 项测试，也不把历史检查代替改稿后的终检。
...
本分支根目录只维护两个文件。论文修改仍应从 LaTeX 源生成 PDF；源文件与实验记录在原开发分支/工作副本保存，不因本分支精简而删除。`paper` 是文档交付入口，不与 `main` 混合发布代码。

每轮先从 `paper` 建新的修改分支，完成后提交 Git，再以正常快进或合并更新 `paper`，不强推覆盖历史。更新 PDF 时同步本文件的版本、哈希和问题状态；不要再增加 `final2`、`v12-final` 等平行文件。关闭事项写明验证结果，新增问题归入 P0/P1/P2。只有问题清单变化时，注明 PDF 未变。

| 日期 | PDF 版本 | 本次变化 |
|---|---|---|
| 2026-09-14 | v11，内容未变 | 建立 `paper` 文档入口；合并当前审计与 96 条外部审阅意见，区分待办、已处理与必须保留的事实；PDF 固定命名 `tang.pdf`。 |

### 可追溯信息

- PDF 对应源提交：`3841a85d00ed4487be2cca7e1022b2279d079d79`。
- 原交付审计提交：`a190b11`；原开发分支：`codex/reltwin-core-only-20260913`。
- 原 LaTeX 路径：`paper/overleaf_icassp2027_reltwin_final_v11`（在原开发工作副本中，不在这个双文件分支内）。
- PDF SHA-256：`7d963ed9b383e9501c7584f39abf6b012a804e178596b2f848f7f2cf8fe819d6`。
