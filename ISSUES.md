...
**当前是 v11 图表更新稿，不是已完成投稿检查的终稿。** 本分支已更新 Figure 1，并在结果节加入 Figure 2 的配对恢复流向图；Figure 2 使用主表同一批 160 个 seed-0 逆查询对，展示均值表格无法表达的样本转移。正文同时将与主表不一致的 `25 remain incorrect` 更正为 24。修改保存在 `codex/reltwin-figure1-redesign`，尚未合入 `paper`。旧版本由 Git 历史保存。编辑与编译方法见 [README.md](README.md)。
...
- 位置：PDF 第 4–5 页。
- 现状：技术内容在第 4 页结束；加入 Figure 2 后，参考文献 [1]–[9] 位于第 4 页，第 5 页列出 [10]–[17]。尚未形成作者要求的“前四页正文、第五页集中放文献”。
...
- 现状：Table 1 仍浮动到方法部分附近，早于结果段的首次引用；本轮缩紧主图后 4.3 小节从第 3 页底开始、段落续至第 4 页。标题并非单独悬置，但表格位置和跨页衔接仍待整稿阶段统一整理。
...
- 现状：新 Figure 2 已把 JointPairAcc 的配对转移画为 84 对持续正确、51 对恢复、1 对退化和 24 对持续错误，并同时展示同窗输出由 37/160 降至 1/160；原正文的 25 是与主表不一致的笔误，已更正。92/239/69 的公开集胜平负及两条 IoU 下降至少 0.5 的样本仍缺少错误类型说明。
- 修改：从已有记录核实两条明显退化究竟涉及关系选错、区间边界还是其他错误，再决定是否用一句概括；不再新增重复 Figure 2 的计数句。
...
- 验证：`overview_data.tex` 与基础版本完全一致；目标与预测条形仍按线性时间轴绘制。第二轮风格版用浅色对角单元格标正确候选、上/下箭头标训练偏好；虚线仅标训练区域，空心目标/实心预测与 set-IoU 的语义不变。独立图和五页论文均编译通过，完成三轮渲染、实际双栏尺寸及灰度检查；底部为 candidate-free inference，图注明确推理未变。
- 原审阅：#55–#57。
...
- 当前 PDF SHA-256：`71f576876ba3b6458b41db1c7491a4f1fa027daaaad0b4feb751911989ebc70d`。
